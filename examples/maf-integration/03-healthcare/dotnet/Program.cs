...
// Demonstrates AGT integration with MAF middleware for healthcare AI.
//
...
//   4. Audit Trail           — SHA-256 Merkle-chained tamper-proof HIPAA compliance log
//
// Usage:
...
static class LlmClient
{
    ...
    static string SimulatedResponse(string prompt, string system = "")
    {
        ...
        if (p.Contains("treatment") && p.Contains("diabetes"))
            return "For Type 2 diabetes, first-line treatment includes lifestyle modifications " +
        ...
        if (p.Contains("drug interaction") || p.Contains("interaction"))
            return "Warfarin + Aspirin: HIGH RISK interaction. Concurrent use significantly " +
        ...
        if (p.Contains("symptom") || p.Contains("headache"))
            return "Persistent headache with blurred vision differential diagnosis: " +
        ...
        if (p.Contains("hypertension") || p.Contains("treatment guideline"))
            return "JNC-8 Hypertension Guidelines: Target BP <140/90 for general population, " +
        ...
        if (p.Contains("ssn") || p.Contains("insurance id") || p.Contains("medical record number"))
...
{
    public static string LookupSymptoms(string description) =>
        JsonSerializer.Serialize(new
        {
            symptoms = description,
            possible_conditions = new[] { "Migraine with aura", "Hypertensive urgency",
                "Intracranial hypertension", "Temporal arteritis" },
            urgency = "moderate",
            recommendation = "BP check, fundoscopy, consider CT/MRI if red flags present"
        });

    public static string CheckDrugInteractions(string drugA, string drugB) =>
        JsonSerializer.Serialize(new
        {
            drug_a = drugA,
            drug_b = drugB,
            interaction_level = "HIGH",
            description = $"{drugA} + {drugB}: increased bleeding risk",
            recommendation = "Monitor INR closely, use lowest effective dose"
        });

    public static string GetTreatmentGuidelines(string condition) =>
        JsonSerializer.Serialize(new
        {
            condition,
    ...
    public static string AccessPatientRecord(string patientId) =>
        "{\"error\": \"This function should never execute — blocked by governance\"}";

    public static string PrescribeMedication(string patientId, string drug, string dosage)
    {
        var controlled = new[] { "oxycodone", "fentanyl", "morphine", "hydrocodone", "adderall", "xanax" };
        if (controlled.Any(c => drug.Equals(c, StringComparison.OrdinalIgnoreCase)))
            return JsonSerializer.Serialize(new
            {
                error = $"Controlled substance '{drug}' requires physician override"
            });
        var match = Regex.Match(dosage, @"\d+");
        if (match.Success && int.Parse(match.Value) > 50)
            return JsonSerializer.Serialize(new
            {
                error = $"Dosage {dosage} exceeds safety limit — requires physician review"
            });
        return JsonSerializer.Serialize(new
        {
            patient_id = patientId, drug, dosage, status = "prescribed", reference = "RX-2024-00891"
        });
    }

    public static string AccessRadiologyRecords(string patientId) =>
        "{\"error\": \"This function should never execute — blocked by governance\"}";

    public static string AccessBillingRecords(string patientId) =>
        "{\"error\": \"This function should never execute — blocked by governance\"}";
}
...
Console.WriteLine($"\n  {Display.Cyan}\U0001f517 LLM Backend:{Display.Reset} {Display.Bold}{backendName}{Display.Reset}");
...
Console.WriteLine($"  {Display.Cyan}\U0001f4cb Policy:{Display.Reset} {engine.Name} ({engine.Rules.Count} rules loaded)");

var audit = new AuditTrail();
var policyMw = new GovernancePolicyMiddleware(engine, audit);
var capabilityMw = new CapabilityGuardMiddleware(
    allowedTools: new[] { "lookup_symptoms", "check_drug_interactions", "get_treatment_guidelines", "prescribe_medication" },
    deniedTools: new[] { "access_patient_record", "access_radiology_records", "access_billing_records" },
    audit: audit);
...
Display.Section("Act 1: Policy Enforcement (HIPAA Compliance)");
...
{
    Display.Request(msg);
    var (allowed, decision) = policyMw.Process("clinical-assistant-agent", msg);
    Display.Policy(engine.Name, decision.RuleName);

    if (allowed)
    {
        Display.Allowed("Forwarding to LLM...");
        var response = chat(msg, "You are an AI clinical assistant at MedAssist Healthcare. Provide evidence-based medical guidance.");
...
Display.DimLine("Tool allow/deny lists restrict which clinical APIs the agent can call");
Console.WriteLine();

var toolTests = new (string Name, Func<string> Fn, string? Extra)[]
{
    ("lookup_symptoms", () => HealthcareTools.LookupSymptoms("persistent headache, blurred vision"), null),
    ("check_drug_interactions", () => HealthcareTools.CheckDrugInteractions("warfarin", "aspirin"), null),
    ("get_treatment_guidelines", () => HealthcareTools.GetTreatmentGuidelines("hypertension"), null),
    ("access_patient_record", () => HealthcareTools.AccessPatientRecord("P-4521"), null),
    ("prescribe_medication", () => HealthcareTools.PrescribeMedication("P-4521", "oxycodone", "80mg"), "controlled + high dose"),
    ("access_radiology_records", () => HealthcareTools.AccessRadiologyRecords("P-4521"), null),
    ("access_billing_records", () => HealthcareTools.AccessBillingRecords("P-4521"), null),
};
...
// Phase A — baseline
Display.Info("\U0001f4ca Phase A: Establishing baseline (5 normal clinical queries)...");
var normalTools = new[] { "lookup_symptoms", "check_drug_interactions", "lookup_symptoms",
...
// Phase B — burst of access_patient_record calls (exfiltration pattern)
Display.Warning("⚡ Phase B: Sudden burst — 20 rapid access_patient_record() calls...");
Display.DimLine("   (Simulating bulk PHI access — data exfiltration pattern)");
bool anomalyDetected = false, quarantineTriggered = false;
AnomalyScore finalScore = new(0, 0, 0, false, false);

for (int i = 0; i < 20; i++)
{
    var patientId = $"P-{1000 + i}";
    var score = rogueMw.RecordCall("access_patient_record");
    audit.Log("clinical-assistant-agent", "tool_call", "anomaly_check", $"access_patient_record({patientId})");
    Thread.Sleep(20);
...
Console.WriteLine();
Console.WriteLine($"  {Display.Yellow}\U0001f4ca Anomaly Analysis:{Display.Reset}");
...
{
    Console.WriteLine();
    Console.WriteLine($"  {Display.Red}{Display.Bold}\U0001f512 QUARANTINE TRIGGERED{Display.Reset} — Agent isolated from production pipeline");
...
Display.DimLine("SHA-256 Merkle-chained log provides tamper-proof HIPAA compliance records");
Console.WriteLine();

Console.WriteLine($"  {Display.Cyan}\U0001f4dc Merkle Chain:{Display.Reset} {audit.Entries.Count} entries\n");

int showCount = Math.Min(4, audit.Entries.Count);
for (int i = 0; i < showCount; i++)
{
    var e = audit.Entries[i];
    var icon = e.Action == "allow" ? "✅" : e.Action == "deny" ? "❌" : "\U0001f4dd";
...
// Verify
Console.WriteLine($"\n  {Display.Cyan}\U0001f50d Integrity Verification:{Display.Reset}");
...
// Proof
Console.WriteLine($"\n  {Display.Cyan}\U0001f4c4 Proof Generation (entry #1):{Display.Reset}");
...
Console.WriteLine($"  {Display.Cyan}\U0001f4dc Audit log: {audit.Entries.Count} entries (Merkle-chained){Display.Reset}");
