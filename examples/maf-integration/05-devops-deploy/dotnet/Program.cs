...
// Demonstrates AGT integration with MAF middleware for CI/CD pipeline safety.
//
...
class RogueDetectionMiddleware
{
    ...
    public AnomalyScore RecordCall(string toolName)
    {
        ...
        bool anomalous = zScore > _zThreshold || capDev > 0.7;
        bool quarantine = zScore > _zThreshold * 1.5 || (anomalous && capDev > 0.74);
...
static class LlmClient
{
    ...
    static string SimulatedResponse(string prompt, string system = "")
    {
        ...
        if (p.Contains("trigger") && p.Contains("build"))
            return "Build triggered successfully for payments-api on develop branch. " +
                   "Build ID: BUILD-9876. Estimated completion: 4 minutes. " +
                   "You can check status with 'check_pipeline_status(BUILD-9876)'.";
        if (p.Contains("status") && (p.Contains("pipeline") || p.Contains("build")))
            return "Pipeline BUILD-5678 status: ✅ Succeeded. " +
                   "Duration: 3m 42s. All 156 tests passed. " +
                   "Artifacts published to staging registry.";
        if (p.Contains("deploy") && (p.Contains("staging") || p.Contains("dev") || p.Contains("test")))
            return "Deployment to staging completed successfully. " +
                   "Service: payments-api v2.1.0. Health checks: passing. " +
                   "Endpoint: https://staging.contoso.com/payments-api";
        if (p.Contains("deploy") && p.Contains("production"))
            return "[This response would never be generated — blocked by policy]";
        if (p.Contains("drop") || p.Contains("truncate") || p.Contains("force push"))
            return "[This response would never be generated — blocked by policy]";
        if (p.Contains("secret") || p.Contains("connection string") || p.Contains("api key"))
            return "[This response would never be generated — blocked by policy]";
        if (p.Contains("test") || p.Contains("run tests"))
            return "Integration test suite completed: 47/47 passed (0 failed, 0 skipped). " +
                   "Coverage: 89.2%. All critical paths validated.";
        if (p.Contains("log"))
            return "Pipeline BUILD-9876 logs: [Step 1/5] Checkout ✓ [Step 2/5] Restore ✓ " +
...
static class DevOpsTools
{
    ...
    public static string TriggerBuild(string repo, string branch) =>
        JsonSerializer.Serialize(new
        {
            build_id = $"BUILD-{Rng.Next(9000, 9999)}",
            repo,
            branch,
            status = "queued",
    ...
    public static string CheckPipelineStatus(string pipelineId) =>
        JsonSerializer.Serialize(new
        {
            ...
            tests_failed = 0,
            coverage = "89.2%",
            artifacts = new[] { "payments-api-2.1.0.tar.gz" }
        });

    public static string DeployToStaging(string service, string version) =>
        JsonSerializer.Serialize(new
        {
            service,
            version,
            environment = "staging",
            status = "deployed",
            endpoint = $"https://staging.contoso.com/{service}",
            health_check = "passing"
        });

    public static string DeployToProduction(string service, string version) =>
        "{\"error\": \"This function should never execute — blocked by governance\"}";

    public static string RunTests(string suite, string environment) =>
        JsonSerializer.Serialize(new
        {
            suite,
            environment,
    ...
    public static string ViewLogs(string pipelineId) =>
        JsonSerializer.Serialize(new
        {
            pipeline_id = pipelineId,
            log_lines = new[]
    ...
    public static string ExecuteDbCommand(string command)
    {
        var upper = command.ToUpperInvariant();
        if (upper.Contains("DROP") || upper.Contains("TRUNCATE") || upper.Contains("DELETE FROM"))
            return JsonSerializer.Serialize(new { error = $"Destructive command blocked: {command}" });
        return JsonSerializer.Serialize(new { command, status = "executed", rows_affected = 0 });
    }

    public static string AccessProductionSecrets(string secretName) =>
        "{\"error\": \"This function should never execute — blocked by governance\"}";

    public static string ForcePush(string repo, string branch) =>
        "{\"error\": \"This function should never execute — blocked by governance\"}";

    public static string DeleteResourceGroup(string rgName) =>
        "{\"error\": \"This function should never execute — blocked by governance\"}";
}
...
var capabilityMw = new CapabilityGuardMiddleware(
    allowedTools: new[] { "trigger_build", "check_pipeline_status", "deploy_to_staging",
                          "run_tests", "view_logs", "execute_db_command" },
    deniedTools: new[] { "deploy_to_production", "access_production_secrets",
                         "force_push", "delete_resource_group" },
    audit: audit);
...
{
    Display.Request(msg);
    var (allowed, decision) = policyMw.Process("deploy-bot-agent", msg);
    Display.Policy(engine.Name, decision.RuleName);

    if (allowed)
    {
        Display.Allowed("Forwarding to LLM...");
        var response = chat(msg, "You are DeployBot, an AI DevOps assistant at Contoso Engineering. Be concise and professional.");
...
Display.DimLine("Tool allow/deny lists restrict which pipeline APIs the agent can call");
Console.WriteLine();

var toolTests = new (string Name, Func<string> Fn, string? Extra)[]
{
    ("trigger_build", () => DevOpsTools.TriggerBuild("payments-api", "develop"), null),
    ("check_pipeline_status", () => DevOpsTools.CheckPipelineStatus("BUILD-9876"), null),
    ("deploy_to_staging", () => DevOpsTools.DeployToStaging("payments-api", "2.1.0"), null),
    ("run_tests", () => DevOpsTools.RunTests("integration", "staging"), null),
    ("view_logs", () => DevOpsTools.ViewLogs("BUILD-9876"), null),
    ("deploy_to_production", () => DevOpsTools.DeployToProduction("payments-api", "2.1.0"), null),
    ("execute_db_command", () => DevOpsTools.ExecuteDbCommand("DROP DATABASE payments_prod"), "destructive"),
    ("access_production_secrets", () => DevOpsTools.AccessProductionSecrets("DB_CONNECTION_STRING"), null),
    ("force_push", () => DevOpsTools.ForcePush("payments-api", "main"), null),
    ("delete_resource_group", () => DevOpsTools.DeleteResourceGroup("prod-payments-rg"), null),
};
...
Display.DimLine("Deployment storm detection — rapid-fire deploys suggest a compromised pipeline");
Console.WriteLine();

// Phase A — baseline
Display.Info("📊 Phase A: Establishing baseline (5 normal pipeline operations)...");
var normalTools = new[] { "trigger_build", "check_pipeline_status", "deploy_to_staging",
                          "run_tests", "view_logs" };
...
// Phase B — deployment storm
var services = new[] {
    "payments-api", "auth-service", "user-service", "billing-api",
    "notification-svc", "analytics-api", "gateway-api", "order-service",
    "inventory-svc", "cache-svc", "audit-service", "config-svc",
    "telemetry-api", "scheduler-svc", "search-api"
};
Display.Warning("⚡ Phase B: Deployment storm — 15 rapid deploy_to_production() calls...");
bool anomalyDetected = false, quarantineTriggered = false;
AnomalyScore finalScore = new(0, 0, 0, false, false);

for (int i = 0; i < 15; i++)
{
    var score = rogueMw.RecordCall("deploy_to_production");
    audit.Log("deploy-bot-agent", "tool_call", "anomaly_check", $"deploy_to_production({services[i]})");
    Thread.Sleep(20);
...
{
    Console.WriteLine();
    Console.WriteLine($"  {Display.Red}{Display.Bold}🔒 QUARANTINE TRIGGERED{Display.Reset} — Agent isolated from CI/CD pipeline");
    Console.WriteLine($"     {Display.Dim}Deployment storm detected — human review required before agent can resume{Display.Reset}");
}
Console.WriteLine();
